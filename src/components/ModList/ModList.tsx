...
export default function ModList({ gamePath, onSelectMod }: Props) {
  ...
  const handleDragMouseDown = useCallback(
    (e: React.MouseEvent, key: string) => {
      ...
      console.log("[drag] mousedown start key=", key, "idx=", idx, "displayOrder.length=", displayOrder.length);
      e.preventDefault();
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (targetDisplayIdx === -1) {
        console.log("[drag] mousemove: targetDisplayIdx -1, closestKey=", closestKey);
        return;
      }

      if (!ds.started || targetDisplayIdx !== ds.currentIdx || slotBeforeGroupId !== ds.slotBeforeGroupId) {
        console.log("[drag] mousemove closestKey=", closestKey, "targetDisplayIdx=", targetDisplayIdx, "overGroup=", dragOverGroupRef.current, "draggedGroupId=", draggedGroupId, "slotBeforeGroupId=", slotBeforeGroupId);
      }
    ...
    const handleMouseUp = () => {
      ...
      console.log("[drag] mouseup sourceKey=", ds?.sourceKey, "started=", ds?.started, "sourceIdx=", ds?.sourceIdx, "currentIdx=", ds?.currentIdx, "dragOverGroup=", targetGroupId, "slotBeforeGroupId=", ds?.slotBeforeGroupId);
      setDragState(null);
      if (ds?.started) {
        setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (useGroupTarget) {
        ...
        if (targetIdx !== ds.currentIdx || ds.slotBeforeKey) {
          console.log("[group-drag] mousemove closestGid=", closestGid, "targetIdx=", targetIdx);
        }
        ...
        if (changed) {
          console.log("[group-drag] mousemove cardTarget=", closestCardKey, "above=", closestCardAbove);
        }
        setGroupHeaderDragState((prev) =>
    ...
    const handleMouseUp = () => {
      const ds = groupHeaderDragStateRef.current;
      console.log("[group-drag] mouseup sourceGroupId=", ds?.sourceGroupId, "started=", ds?.started, "sourceIdx=", ds?.sourceIdx, "currentIdx=", ds?.currentIdx, "slotBeforeKey=", ds?.slotBeforeKey);
        ...
        if (ds.slotBeforeKey) {
          ...
          if (group && group.modKeys.length > 0) {
            setDisplayOrder((prev) => {
              ...
              console.log("[group-drag] mouseup moved group cards to displayOrder idx=", idx, "insertAfter=", ds.insertAfter, "moved=", moved.length, "cards");
              return filtered;
            });
            ...
            console.log("[group-drag] mouseup set anchor for empty group, anchorBefore=", ds.insertAfter ? undefined : ds.slotBeforeKey, "anchorAfter=", ds.insertAfter ? ds.slotBeforeKey : undefined);
          }
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (changed) {
        console.log("[group-create] mousemove closestKey=", closestKey, "insertAfter=", insertAfter, "insertBefore=", insertBefore, "groupOrderIdx=", goIdx, "groupOrder.length=", groupOrderRef.current.length, "positions.size=", positions.size, "gHeaders.size=", gHeaders.size, "scrollDelta=", scrollDelta);
    ...
    const handleMouseUp = () => {
      const gc = groupCreateRef.current;
      if (!gc) return;
      console.log("[group-create] mouseup active=", gc.active, "insertAfter=", gc.insertAfter, "insertBefore=", gc.insertBefore, "groupOrderIdx=", gc.groupOrderIdx);
        ...
        (() => {
          ...
          for (let gi = 0; gi < groupOrder.length; gi++) {
            ...
            if (group.anchorBefore) {
              ...
              console.log("[group-create] anchor anchorBefore=", group.anchorBefore, "idx=", idx, "insertAt=", insertAt);
            } else if (group.anchorAfter) {
              const idx = renderItems.findIndex(
                (item) => item.type === "mod" && item.key === group.anchorAfter,
              );
              insertAt = idx === -1 ? renderItems.length : idx + 1;
              console.log("[group-create] anchor anchorAfter=", group.anchorAfter, "idx=", idx, "insertAt=", insertAt, "renderItems.length=", renderItems.length);
          ...
          if (groupCreateState?.active) {
            ...
            console.log("[group-create] render placeholderIdx=", placeholderIdx, "insertAfter=", groupCreateState.insertAfter, "insertBefore=", groupCreateState.insertBefore, "groupOrderIdx=", groupCreateState.groupOrderIdx);
